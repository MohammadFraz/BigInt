...
Bigint::Bigint(const string& str) : s(1), n()
{
  string t(str);
...
Bigint::Bigint(long long a): s(0), n()
{
  if(a==0) return;
...
Bigint Bigint::operator/(const Bigint& b) const
{
  ...
  if(lt(this->n, maxlong) && lt(b.n, maxlong)) // Use fast long long
    ...
    {
      Bigint bb = b.abs();
      vector<Bigint> tab(11);
      for(int i = 0; i < 11; ++i) tab[i] = Bigint(i)*bb;
 
      Bigint ratio(0), resi(1, n.substr(n.size()-b.n.size(),b.n.size()));
      for(int i = n.size()-b.n.size(); i >= 0; --i)
        {
          ...
          ratio = ratio * Bigint(10) + Bigint(count);
          if(count) resi = resi - tab[count];
          
          if(i>0) resi = resi * Bigint(10) + Bigint(n[i-1]-'0');
        }
...
Bigint Bigint::Bpow(int p) const
{
...
long long Bigint::sumOfDigits() const
{
  int nsize = n.size();
  long long sum = 0;
